...
$$V_c = \int_{t_0}^{t_n} Q_j dt = V_{j=n} $$
$$iT_{R0} = t_n - t_0$$
where $V_c$ is the cumulative volume of discharge starting at time $t=0$ up until time $t=n$ where $n$ is the time at which this cumulative discharge is equal to the entire system volume ($V_j$) at time $n$. The integrated residence time ($iT_R$) at $t=0$ is then calculated as the difference between $t_0$ and $t_n$.
